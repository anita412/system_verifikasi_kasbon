        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title" id="1-form-entry">Show {{$nonkasbon->nokasbon}}</h4>
                    <h4 class="card-title" id="2-form-entry">Keterangan</h4>
                </div><!--end card-header-->
                <div class="card-body bootstrap-select-1 form-wizard-wrapper">
                    {!! Form::model($nonkasbon, ['method' => 'PATCH','route' => ['vnk-atasan.update', $nonkasbon->id],'class' => 'form-parsley']) !!}
                        <input value="{{$nonkasbon->id}}" class="text-muted mb-0" name="id" hidden>
                        {{ csrf_field() }}
                        @include('vnk-atasan.edit-cek')
                        @include('vnk-atasan.edit-keterangan')
                       {!! Form::close() !!} 
                </div><!--end card-body-->
            </div><!--end card-->
        </div><!--end col-->
